...
def test_add_review(client, authenticate):
    with client:
        ...
        response = client.post(
            "/review/0316228532",
            data={"rating": "5", "user_review": "Really good book"},
        )
...
def test_add_review_input_errors(rating, review_input, message, client, authenticate):
    with client:
        ...
        client.get("/review/0316228532")
        response = client.post(
            "/review/0316228532", data={"rating": rating, "user_review": review_input}
        )
        assert response.status == "302 FOUND"
        response = client.get("/review/0316228532")
        assert response.status == "200 OK"
...
def test_add_review_invalid_request(client, authenticate):
    with client:
        ...
        client.get("/review/0316228532")
        response = client.post(
            "/review/0", data={"rating": "5", "user_review": "Really good"}
        )
        assert response.status == "302 FOUND"
        response = client.get("/review/0316228532")
        assert response.status == "200 OK"
        ...
        client.get("/review/0316228532")
        response = client.post(
            "/review/0316228532", data={"rating": "5", "user_review": "Really good"}
        )
        response = client.post(
            "/review/0316228532", data={"rating": "5", "user_review": "Really good"}
        )
...
def test_all_reviews(client, authenticate):
    with client:
        ...
        client.post(
            "/review/0099481685", data={"rating": "5", "user_review": "Really good"}
        )
        response = client.get("/your_reviews/1")
        assert response.status == "200 OK"
